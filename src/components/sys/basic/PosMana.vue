...
<template>
  <div>
    ...
    <div class="posManaMain">
      <el-table :data="positions" border stripe size="small" @selection-change="handleSelectionChange"
        style="width: 70%">
...
export default {
  ...
  methods: {
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          this.multipleSelection.forEach(item => {
            ids += "ids=" + item.id + "&";  //拼接请求  ？ids=1&ids=2&
          });
    ...
    addPosition() {
      //添加职位
      if (this.pos.name) {
        // this.postRequest("/system/basic/pos/"（请求地址）, this.pos（传递json对象）)
    ...
    showEditView(index, data) {
      //显示信息
      Object.assign(this.updatePos, data);   //数据拷贝
      this.updatePos.createDate = ""; ///创建日期设置为空

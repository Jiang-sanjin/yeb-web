        ...
        <el-aside width="200px">
          <el-menu router unique-opened>
            <el-submenu :index="index+''" v-for="(item,index) in routes" v-if="!item.hidden" :key="index">
              <template slot="title">
                <!-- 使用font-awesome  样式 -->
                <i style="color: #ADE1CC;margin-right: 5px" :class="item.iconCls"></i>
                <span>{{item.name}}</span>
              </template>
...
export default {
  ...
  //computed 属性称为 计算属性   计算属性允许我们对指定的视图，复杂的值计算。这些值将绑定到依赖项值，只在需要时更新。
  computed: {
    routes() {
      return this.$store.state.routes;   //调用vuex配置   store/index.js  文件
